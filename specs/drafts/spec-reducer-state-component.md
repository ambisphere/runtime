...
ADR-0001 is now **accepted** and publishes the canonical numbered invariant list (1–9) this spec's conformance table anchors to; every "ADR-0001 inv. N" reference here is therefore binding, not provisional. The two amendments this spec depended on were resolved at ADR acceptance:

- **Amendment A (invariant 8 — event types) — incorporated.** ADR-0001 distinguishes **domain event types (zero in core)** from a closed **enumerated runtime-meta allowlist** core reserves (`attention.*`, `approval.*`, `entity.*`, `edge.*`, `ambisphere.egress.performed`). This spec references that allowlist rather than carving its own exception (see conformance table and § attention facet).
- **Amendment B (closed core-component list) — to confirm against the accepted ADR.** This spec proposes the closed set of core-owned component *shapes* (see § "core-owned component types"); confirm the accepted ADR ratifies exactly this list so identity/daemon/persona/bundle specs reference one authoritative enumeration rather than introducing core components ad hoc. Tracked as an open question.
